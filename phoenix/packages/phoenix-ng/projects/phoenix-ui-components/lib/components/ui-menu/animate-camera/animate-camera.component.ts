...
import { SSModeComponent } from '../ss-mode/ss-mode.component';
...
export class AnimateCameraComponent {
  ...
  @ViewChild(SSModeComponent) ssModeComponent: SSModeComponent;
  ...
  /**
   * Enter screenshot (fullscreen) mode, then run the preset animation.
   * Exits screenshot mode automatically when the animation completes.
   */
  animatePresetInSSMode(preset: string) {
    this.ssModeComponent.enterSSMode(() => {
      this.setDetectorOpacity(0.2);
      this.eventDisplay.animatePreset(this.animationPresets[preset], () => {
        this.setDetectorOpacity(1);
        this.ssModeComponent.exitSSMode();
