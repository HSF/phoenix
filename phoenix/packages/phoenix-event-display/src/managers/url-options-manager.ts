...
export class URLOptionsManager {
  ...
  /**
   * If the URL contains `?ss=preset-name`, enter screenshot (fullscreen) mode
   * and run the named preset animation once all assets have loaded.
   * This allows recording with OBS or similar screen capture software by
   * simply sharing a URL.
   *
   * Example: https://my-phoenix/?ss=Cavern+to+ID
   */
  public applySSAnimationOption() {
    const presetName = this.urlOptions.get('ss');
    if (!presetName) return;

    this.eventDisplay.getLoadingManager().addLoadListenerWithCheck(() => {
      // Small delay to let the renderer settle after loading
      setTimeout(() => {
        this.eventDisplay.animatePresetInSSMode(presetName);
      }, 500);
    });
  }
