...
export class EventDisplay {
  ...
  /**
   * Enter screenshot (fullscreen) mode and run a named preset animation.
   * Exits screenshot mode automatically when the animation completes.
   * Designed for recording with OBS or similar screen capture software.
   * Can also be triggered via URL: ?ss=preset-name
   * @param presetName Name of the animation preset to run.
   * @param presets Map of preset name to AnimationPreset. Required to resolve the preset.
   */
  public animatePresetInSSMode(
    presetName: string,
    ...
    if (!preset) {
      console.warn(
        `[Phoenix] animatePresetInSSMode: preset "${presetName}" not found.`,
      );
      return;
    }
    document.body.classList.add('ss-mode');
    document.documentElement.requestFullscreen?.().catch(() => {});
    this.graphicsLibrary.animatePreset(preset, () => {
